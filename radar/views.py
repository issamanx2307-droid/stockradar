...
def _try_ai_reply(user, admin_user, user_message: str):
    ...
    system_prompt = (
        ...
        "═══ กฎเมื่อเกิด Error หรือข้อมูลไม่ครบ (สำคัญมาก) ═══\n"
        "- ถ้า tool คืน data_available=False หรือมี error field → บอกผู้ใช้ตรงๆ ว่า 'ดึงข้อมูลไม่ได้' พร้อมสาเหตุ\n"
        "- ถ้า indicator ใดมีค่า null หรืออยู่ใน _null_fields → บอกว่า 'ไม่มีข้อมูล [ชื่อ indicator]' ห้ามเดาหรืออ้างค่าที่ไม่มีจริง\n"
        "- ห้ามสร้างข้อมูลขึ้นมาเองในทุกกรณี\n"
        "- ถ้าหุ้นไม่อยู่ในระบบ → บอกตรงๆ ว่า 'ไม่พบ [symbol] ในระบบ กรุณาตรวจสอบชื่อหุ้น'\n\n"

        "═══ วิธีแปลผล Indicators ═══\n"
        "- RSI: <35=Oversold, 35-70=ปานกลาง, >70=Overbought\n"
        "- MACD Hist: บวก=โมเมนตัมขึ้น, ลบ=โมเมนตัมลง\n"
        "- ADX: <20=trend อ่อน, 20-25=เริ่มมี trend, >25=trend แข็ง, >40=trend แรงมาก\n"
        "- DI+>DI-=แนวโน้มขึ้น, DI+<DI-=แนวโน้มลง\n"
        "- BB: ราคาใกล้ bb_upper=ใกล้แนวต้าน, ใกล้ bb_lower=ใกล้แนวรับ, bb_middle=เส้น MA20\n"
        "- ATR14: ความผันผวนรายวัน (สูง=ผันผวนมาก ควรระวัง stop loss)\n"
        "- volume_ratio: >1.5=ปริมาณซื้อขายสูงผิดปกติ (สัญญาณยืนยัน)\n\n"

        "═══ ตัวอย่างรูปแบบที่ถูก ═══\n"
        "**PTT — คะแนนดี (ข้อมูลย้อนหลัง 120 วัน)**\n"
        "ราคา: 34.31 บาท | วันที่: 04/04/2025\n\n"
        "**Layer 1 — Trend** ✅ ผ่าน\n"
        "EMA20: 34.31 > EMA50: 33.72 > EMA200: 32.10\n\n"
        "**Layer 2 — Structure** ✅ ผ่าน\n"
        "ใกล้แนวรับ 33.50 (ห่าง 2.4%) แนวต้าน 35.80\n\n"
        "**Layer 3 — Pattern** ❌ ไม่ผ่าน\n"
        "ไม่พบ candlestick pattern ที่ชัดเจน\n\n"
        "**Layer 4 — Momentum** ✅ ผ่าน\n"
        "RSI: 57.2 (มีที่ไป) | MACD Hist: +0.09 (โมเมนตัมบวก)\n\n"
        "**Bollinger Bands**\n"
        "Upper: 35.50 | Middle: 34.00 | Lower: 32.50 (ราคาอยู่กลาง BB)\n\n"
        "**ADX/DMI**\n"
        "ADX: 28.5 (trend แข็ง) | DI+: 22.1 > DI-: 15.3\n\n"
        "**Volume**\n"
        "วันนี้: 12,500,000 หุ้น | เฉลี่ย 20 วัน: 10,000,000 หุ้น (1.25×)\n\n"
