...
def _try_ai_reply(user, admin_user, user_message: str):
    ...
    system_prompt = (
        ...
        "ตัวอย่างการอธิบายที่ถูก:\n"
        "  RSI=57.2 → 'RSI: 57.2 — อยู่ในโซน Bullish (40-65) มีที่ไปอีก 12.8 จุดก่อนถึง Overbought (70)'\n"
        "  BB → 'ราคาอยู่ที่ 55% ของ BB (กลางค่อนไปบน) ห่างแนวต้าน BB Upper 3.2% | BB กว้าง 6.5% (ปกติ)'\n"
        "  ATR → 'ATR=1.15 บาท (3.4% ของราคา) ผันผวนวันละ ~3.4% | Stop Loss แนะนำ 1.5×ATR = 32.58 บาท'\n"
        "  ADX → 'ADX=29 (trend แข็ง) DI+=16 > DI-=11 ห่างกัน 5 จุด แนวโน้มขาขึ้น'\n"
        "  Volume → 'ปริมาณวันนี้ต่ำกว่าเฉลี่ย 15% (0.85×) — สัญญาณน้ำหนักน้อย ควรรอ volume ยืนยัน'\n\n"
        ...
        "**PTT — คะแนนดี (ข้อมูลย้อนหลัง 120 วัน)**\n"
        "ราคา: 34.31 บาท | วันที่: 04/04/2025\n\n"
        "**Layer 1 — Trend** ✅ ผ่าน\n"
        "EMA20(34.31) > EMA50(33.72) > EMA200(32.10) — Strong Uptrend\n"
        "ราคาเหนือ EMA50 อยู่ +1.7% | EMA20 นำ EMA50 อยู่ +1.8%\n\n"
        "**Layer 2 — Structure** ✅ ผ่าน\n"
        "ใกล้แนวรับ 33.50 (ห่าง 2.4%) | แนวต้าน 35.80 (ห่าง 4.3%)\n\n"
        "**Layer 3 — Pattern** ❌ ไม่ผ่าน\n"
        "ไม่พบ candlestick pattern ที่ชัดเจน\n\n"
        "**Layer 4 — Momentum** ✅ ผ่าน\n"
        "RSI: 57.2 — โซน Bullish มีที่ไปอีก 12.8 จุดก่อน Overbought\n"
        "MACD Hist: +0.09 (momentum บวก) MACD เหนือ Signal\n\n"
        "**Bollinger Bands**\n"
        "Upper: 35.50 | Middle: 34.07 | Lower: 32.64\n"
        "ราคาอยู่ที่ 55% ของ BB — กลางค่อนบน ห่างแนวต้าน BB 3.2% | BB กว้าง 8.4%\n\n"
        "**ATR — ความผันผวน**\n"
        "ATR14: 1.15 บาท (3.4% ต่อวัน) — ปกติ\n"
        "Stop Loss แนะนำ 1.5×ATR: 32.58 บาท (ความเสี่ยง 1.73 บาท/หุ้น)\n\n"
        "**ADX/DMI — ความแข็งแกร่ง Trend**\n"
        "ADX: 29 (trend แข็ง) | DI+: 16.3 > DI-: 11.6 (แนวโน้มขึ้น ห่าง 4.7 จุด)\n\n"
        "**Volume**\n"
        "วันนี้: 96,804,895 หุ้น | เฉลี่ย 20 วัน: 113,845,784 หุ้น\n"
        "ต่ำกว่าเฉลี่ย 15% (0.85×) — ปริมาณเบาบาง สัญญาณน้ำหนักน้อย\n\n"
        "**Range 20 วัน**\n"
        "ราคาอยู่ที่ 62% ของ Range (ต่ำสุด 30.50 — สูงสุด 36.75)\n"
        "ห่างจุดสูงสุด 7.1% | ห่างจุดต่ำสุด 12.5%\n\n"
        "**สรุป:** ผ่าน 3 ใน 4 Layer\n\n"

        "═══ เมนูในระบบ radarhoon.com ═══\n"
        "ระบบมีเมนูหลักดังนี้ (สามารถอธิบายการใช้งานให้ user ได้):\n\n"
        "1. **Fundamental** — วิเคราะห์ปัจจัยพื้นฐานของหุ้น: P/E, P/BV, ROE, EPS, Dividend Yield\n"
        "   ใช้สำหรับ: ประเมินมูลค่าหุ้น เปรียบเทียบกับอุตสาหกรรม\n\n"
        "2. **หุ้น VI** — หน้าจอกรองหุ้น Value Investing\n"
        "   ใช้สำหรับ: หาหุ้นที่ราคาต่ำกว่ามูลค่าที่แท้จริง (undervalued)\n\n"
        "3. **Multi-Layer Scanner** — สแกนหุ้นด้วย 4 Layer: Trend, Structure, Pattern, Momentum\n"
        "   Layer 1 Trend: EMA20>EMA50>EMA200 (ขาขึ้น)\n"
        "   Layer 2 Structure: ราคาใกล้แนวรับ/แนวต้านที่ดี\n"
        "   Layer 3 Pattern: Candlestick pattern เช่น Hammer, Engulfing, Doji\n"
        "   Layer 4 Momentum: RSI + MACD + Volume ยืนยัน\n"
        "   ผ่านมากกว่า 3/4 Layer = สัญญาณที่ดี\n\n"
        "4. **สแกนหุ้น** — สแกนหุ้นทั้งตลาดตาม signal เงื่อนไขต่างๆ\n"
        "   ใช้สำหรับ: หาหุ้นที่ตรงกับ criteria ที่กำหนด\n\n"
        "5. **กราฟ** — ดูกราฟราคาพร้อม Indicators: EMA, BB, RSI, MACD, Volume\n"
        "   ใช้สำหรับ: วิเคราะห์ทางเทคนิค ดู trend และจุดเข้า/ออก\n\n"
        "6. **Backtest** — ทดสอบกลยุทธ์การเทรดย้อนหลัง\n"
        "   ใช้สำหรับ: ประเมินประสิทธิภาพของกลยุทธ์ก่อนใช้จริง\n\n"
        "7. **กลยุทธ์** — จัดการและสร้างกลยุทธ์การเทรดของตัวเอง\n\n"
        "8. **ค้นเมนู & งาน-ตอน** — ดูประวัติและงานต่างๆ\n\n"

        "═══ การอ่านค่า Backtest ═══\n"
        "ระบบ Backtest ทดสอบกลยุทธ์ย้อนหลังและแสดงผลดังนี้:\n\n"
        "**ระยะเวลาที่ควรทดสอบ:**\n"
        "  • 1-2 ปี: เหมาะกับกลยุทธ์ระยะสั้น swing trade — ได้ข้อมูลเร็ว แต่อาจไม่ครอบคลุมหลายสภาพตลาด\n"
        "  • 3-5 ปี: แนะนำ — ครอบคลุมทั้งตลาดขาขึ้น ขาลง และ sideways\n"
        "  • 5+ ปี: เหมาะกับกลยุทธ์ระยะยาว แต่ต้องระวัง market regime เปลี่ยน\n"
        "  ⚠️ Backtest ยิ่งนาน ยิ่งน่าเชื่อถือ แต่ต้องระวัง overfitting\n\n"
        "**ค่าที่ต้องดู (สำคัญมาก):**\n"
        "  win_rate (%)       : อัตราการชนะ >50% = ดี, >60% = ดีมาก\n"
        "  profit_factor      : รวมกำไร ÷ รวมขาดทุน\n"
        "                       1.5+ = ดี, 2.0+ = ดีมาก, <1.0 = ขาดทุน\n"
        "  max_drawdown (%)   : ขาดทุนสูงสุดจาก peak ก่อนฟื้น\n"
        "                       <10% = ดี, <20% = พอรับได้, >30% = เสี่ยงสูง\n"
        "  sharpe_ratio       : ผลตอบแทนต่อความเสี่ยง\n"
        "                       >1.0 = ดี, >2.0 = ดีมาก, <0 = แย่กว่า risk-free\n"
        "  total_return (%)   : ผลตอบแทนรวม — ต้องเทียบกับ buy_hold_return\n"
        "  buy_hold_return (%) : ถ้าซื้อแล้วถือตลอด ได้เท่าไหร่ (benchmark)\n"
        "  total_trades       : จำนวนการเทรด — น้อยเกินไป (<10) ไม่น่าเชื่อถือ\n\n"
        "**การอ่านผลรวม:**\n"
        "  กลยุทธ์ดี = win_rate>50% + profit_factor>1.5 + max_drawdown<20% + sharpe>1\n"
        "  และ total_return ต้องชนะ buy_hold_return\n"
        "  ถ้า total_return < buy_hold_return = กลยุทธ์แพ้การถือหุ้นธรรมดา\n\n"
...
def alpaca_account(request):
    ...
    ดูยอดเงิน equity, buying_power, cash ของ Alpaca account
    """
    try:
...
def alpaca_orders(request):
    ...
    ดูรายการ orders จาก Alpaca  status: open | closed | all
    """
    ...
    try:
        ...
        # รวม pending_confirm orders จาก DB ของเราด้วย
        local_pending = list(
...
def alpaca_propose_order(request):
    ...
    บันทึก order ที่ AI เสนอ (ยังไม่ส่งไป Alpaca) รอ user confirm

    Body: { symbol, side, qty, order_type, limit_price(opt), ai_reasoning }
    """
    data = request.data
...
def alpaca_confirm_order(request, order_id):
    ...
    User ยืนยัน → ส่ง order ไป Alpaca จริง
    """
    try:
...
def alpaca_cancel_order(request, order_id):
    ...
    ยกเลิก order — ถ้ายัง pending_confirm ยกเลิกแค่ใน DB
    ถ้า submitted แล้ว ส่งคำสั่งยกเลิกไป Alpaca ด้วย
    """
    try:
...
def alpaca_portfolio(request):
    ...
    ดู P&L ย้อนหลัง  period: 1D|1W|1M|3M|1A
    """
